...
export default function App() {
  ...
  // WebSocket event handlers (existing code remains the same)
  useEffect(() => {
  ...
  // Player join/leave handlers (existing code remains the same)
  useEffect(() => {
    if (!webSocketService || !currentMultiplayerRoom) return;
  ...
  style={{
    ...
    backgroundPosition: reducedMotion
      ? 'center' // Static position when reduced motion
...
//
//  const prettyName = (raw: any): string => {
//    // 1. authenticated user  -> use username
//    if (raw.username && typeof raw.username === 'string') return raw.username;
//    // 2. guest user          -> guest-8chars
//    const id = raw.userId || raw.id;
//    if (id) return `guest-${id.slice(0, 8)}`;
//    // 3. last resort
//    return 'Someone';
...
//  const handlePlayerJoined = (payload: any) => {
//    const id = payload.userId || payload.id;
//    if (!id) return;                       // malformed event
//
//    const name = prettyName(payload);
//
//    // --- toast only if it is NOT me ---
//    if (id !== myId) {
//      setToasts(prev => [...prev, {
//        id: Date.now().toString(),
//        message: `${name} joined the room`,
